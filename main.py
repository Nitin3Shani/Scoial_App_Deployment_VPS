...
from starlette.exceptions import HTTPException as StartHTTPException
...
async def home(request: Request, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    result = await db.execute(
        select(models.Post).options(selectinload(models.Post.author)).order_by(models.Post.date_posted.desc()).limit(settings.posts_per_page)
    )
...
@app.exception_handler(StartHTTPException)
async def general_http_exception_handler(request: Request, exception: StartHTTPException):
